...
import cv2, time, os, torch, requests
        ...
        for i, det in enumerate(results):
            ...
            if det is not None and len(det):
                for *xyxy, conf, cls in reversed(det):
                    ...
                    score = "%.2f" % conf
                    x1, y1, x2, y2 = int(xyxy[0] * w / self.input_width), int(xyxy[1] * h / self.input_width), int(
                        xyxy[2] * w / self.input_width), int(xyxy[3] * h / self.input_width)
                    cc.append({"type": label, "score": score, "xmin": x1, "ymin": y1, "xmax": x2, "ymax": y2,
                               "time": inference_time})
                return cc
