...
1. SFT(Supervised Fine-Tuning)监督微调
  ...
  1. 2 (全是干货)[https://cloud.tencent.com/developer/article/2338313]


- ## yolo用什么计算损失
  - YOLO（You Only Look Once）系列的目标检测模型在计算损失时，通常考虑多个方面，包括目标框的定位损失、目标框的置信度损失以及分类损失。具体来说，YOLO预测每个网格单元有多个边界框，并使用预测值与基本事实之间的平方和误差来计算损失。以下是YOLO系列模型中计算损失的主要方面：

  - 边界框回归损失：这部分损失主要关注模型预测的边界框与真实边界框之间的差异。不同的YOLO版本可能使用不同的损失函数来计算这部分损失。
  - 置信度损失：置信度损失衡量的是模型对于预测边界框内是否存在目标的信心。这部分损失通常使用二元交叉熵损失函数进行计算。
  - 分类损失：对于目标分类任务，YOLO模型会计算分类损失，即模型预测的类别与真实类别之间的差异。这部分损失也通常使用二元交叉熵损失函数进行计算。
  此外，为了解决YOLO对同一物体进行重复检测的问题，通常会应用非最大抑制（NMS）技术，以较低的置信度删除重复项。非最大抑制可以在一定程度上提高模型的性能。

- ## 怎么解释BCE,CE
  - BCE（Binary Cross Entropy，二元交叉熵）用于处理二分类问题，例如预测某个事件是否会发生（如明天是否下雨）。在二分类问题中，目标标签通常是0或1，代表两个可能的类别。BCE损失函数通过计算预测概率与实际标签之间的差异来衡量模型的性能。当实际标签为0时，损失函数鼓励模型预测概率尽可能接近0；当实际标签为1时，损失函数鼓励模型预测概率尽可能接近1。

  - CE（Categorical Cross Entropy，类别交叉熵）则用于处理多分类问题，即预测一个样本属于多个可能类别中的哪一个。在多分类问题中，目标标签通常是一个one-hot编码的向量，表示样本属于哪个类别。CE损失函数计算模型预测的概率分布与实际标签的概率分布之间的差异。通过最小化CE损失，模型可以学习到如何更好地将样本分类到正确的类别中。

- ## 用什么样的一个模型去训练embedding
  - 训练embedding的模型可以根据具体的应用场景和数据类型来选择。以下是一些常见的模型和方法：

  - Word2Vec：这是一种基于神经网络的NLP嵌入模型训练方法，适用于文本数据。通过分析大量文本数据中的上下文关系，Word2Vec将单词映射到连续向量空间中的固定维度。它有两种训练算法：Skip-gram和CBOW。Skip-gram更适用于大型数据集，而CBOW对小型数据集效果更好。
  - GloVe：GloVe是一种基于全局词汇共现矩阵的NLP嵌入模型训练方法。它使用全局上下文统计信息，通过最小化单词之间的共现矩阵的差异，得到每个单词的嵌入表示。GloVe模型可以有效地捕捉到单词的语义和语法信息。
  - FastText：FastText也是一种基于Word2Vec的NLP嵌入模型训练方法，但它还支持子词嵌入，能够更好地处理复杂的单词构成。
  - BERT：BERT是一种基于Transformer的NLP嵌入模型训练方法，近年来取得了重大突破。BERT通过预训练大量的文本数据，可以生成高质量的词嵌入，适用于各种NLP任务。

- ## 大模型的衡量指标是什么
  - 模型可重现性：从统计角度，使用一致性和准确性来衡量模- 型的可重现性，评估模型可重复用途的能力。
  - 模型的准确性：通过R^2值、解释方差等指标来衡量模型的准确性。此外，准确率（Accuracy）也是衡量模型预测正确性的重要指标，它指的是模型在训练数据集中成功识别样本的比例。
  - 模型的可解释性：用模型提取的变量可以被解释，从而辅助决策。
  - 模型的可靠性：评估模型的结果是否可靠，是否可能影响模型的结果。
